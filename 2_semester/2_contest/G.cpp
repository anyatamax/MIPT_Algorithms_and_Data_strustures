...
double ToLine(Point& point, Point& first, Point& second, Vector& vector) {
    double result = static_cast<double>(vector[1] * point.GetX() - vector[0] * point.GetY() + first.GetY()
                                                                                              * vector[0] - first.GetX() * vector[1])
...
double ToSegment(Point& point, Point& first, Point& second, Vector& vector, Vector& vector_reverse) {
    Vector first_to_point(first, point);
    Vector second_to_point(second, point);
    if (point.GetX() == first.GetX() && point.GetY() == first.GetY()) {
        return 0;
    }
    if (point.GetX() == second.GetX() && point.GetY() == second.GetY()) {
        return 0;
    }
...
int main() {
    ...
    Point first_seg1(point_first_x, point_first_y);
    Point second_seg1(point_second_x, point_second_y);
    Segment seg_first(first_seg1, second_seg1);
    Vector vector_first(first_seg1, second_seg1);
    Vector vector_first_reverse(second_seg1, first_seg1);
    std::cin >> point_first_x >> point_first_y >> point_second_x >> point_second_y;
    Point first_seg2(point_first_x, point_first_y);
    Point second_seg2(point_second_x, point_second_y);
    Segment seg_second(first_seg2, second_seg2);
    Vector vector_second(first_seg2, second_seg2);
    Vector vector_second_reverse(second_seg2, first_seg2);
    if (seg_first.CrossesSegment(seg_second)) {
        double distance = 0;
        std::cout << std::fixed << std::setprecision(6) << distance;
    } else {
        double distance = std::min(std::min(ToSegment(first_seg1, first_seg2, second_seg2,
                                                      vector_second, vector_second_reverse),
                                            ToSegment(second_seg1, first_seg2, second_seg2,
                                                      vector_second, vector_second_reverse)),
                                   std::min(ToSegment(first_seg2, first_seg1, second_seg1,
                                                      vector_first, vector_first_reverse),
                                            ToSegment(second_seg2, first_seg1, second_seg1,
                                                      vector_first, vector_first_reverse)));
        std::cout << std::fixed << std::setprecision(6) << distance;
    }
    return 0;
